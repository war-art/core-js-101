...
function timeSpanToString(startDate, endDate) {
  ...
  const pad = (v, number) => v.toString().padStart(number, '0');
  let dif = endDate - startDate;
  const hoursDif = Math.floor(dif / hour);
  dif -= hoursDif * hour;
  const minutesDif = Math.floor(dif / minutes);
  dif -= minutesDif * minutes;
  const secondsDif = Math.floor(dif / seconds);
  const mSecondsDif = dif - secondsDif * seconds;

  return `${pad(hoursDif, 2)}:${pad(minutesDif, 2)}:${pad(secondsDif, 2)}.${pad(mSecondsDif, 3)}`;
}
